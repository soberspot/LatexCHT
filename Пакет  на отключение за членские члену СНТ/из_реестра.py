...
# читает файл excel
def FileExcel(file, list):     
    src_file = Path.cwd() / file
    df = pd.read_excel(src_file, sheet_name=list, header=0, usecols='A:Z')
    return df


# из полного ФИО собственника оставляет имя и отчество
def _split_fio(df, strc):    
...
#  Выборка по немерам участков (1. Делаем номер участка индексом  2. По индексу выбираем данные из датафрейм. 3. Выводим н печать)
homesteads = [472]                               
